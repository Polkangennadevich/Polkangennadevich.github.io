...
( function( window, $, undefined ) {
	...
	$.Cassette.prototype 	= {
		...
		_createPlayer		: function() {
            ...
            this.$wheelLeft1 = this.$theTape.find( 'div.vc-tape-wheel-left1' );
			this.$wheelRight= this.$theTape.find( 'div.vc-tape-wheel-right' );
            this.$wheelRight1 = this.$theTape.find( 'div.vc-tape-wheel-right1' );
		...
		_createControls		: function() {
			...
			this.$cSwitch	= $( '<li class="vc-control-switch">&nbsp;Play List<span></span></li>' );
			...
			if (document.createElement('audio').canPlayType) {
				...
				if (!document.createElement('audio').canPlayType('audio/ogg') && !document.createElement('audio').canPlayType('audio/mpeg')) {
		...
		_getWheelValues		: function( x ) {
			...
			var T	= this._getSide().current.getDuration(),
				val = {
					left	: ( this.currentSide === 1 ) ? ( -85 / T ) * x + 85 : ( 85 / T ) * x,
					right	: ( this.currentSide === 1 ) ?  ( 85 / T ) * x : ( -85 / T ) * x + 85
				};
			return val;
		...
		_getPosTime			: function() {
			...
			if( wright1 === undefined ) {

				wright1 = 0;
				
			}
			var T		= this._getSide().current.getDuration(),
				posTime	= this.currentSide === 2 ? ( T * wleft ) / 85 : ( T * wright ) / 85;
		...
		_updateWheelValue	: function( wheelVal ) {
			...
			this.$wheelLeft.data( 'wheel', wheelVal.left ).css( { 'box-shadow' : '0 0 0  '  + wheelVal.left + 'px rgb(136, 61, 41)' } );
			this.$wheelRight.data( 'wheel', wheelVal.right ).css( { 'box-shadow' : '0 0 0 ' + wheelVal.right + 'px rgb(136, 61, 41)' } );
		...
		_setWheelAnimation	: function( speed, mode ) {
			...
			setTimeout( function() {
                ...
                _self.$wheelLeft1.css(animStyle);
				_self.$wheelRight.css(animStyle);
                _self.$wheelRight1.css(animStyle);
		...
		_stopWheels			: function() {
            ...
            this.$wheelLeft1.css( wheelStyle );
			this.$wheelRight.css( wheelStyle );
            this.$wheelRight1.css( wheelStyle );
	...
	$.Song.prototype 		= {
		...
		_init				: function() {
			...
			this.sources	= {
                ogg	: 'TapeRecorder/songs/' + this.name + '.ogg',
				mp3	: 'TapeRecorder/songs/' + this.name + '.mp3'
	...
	$.Sound.prototype		= {
		...
		getSource			: function( type ) {
			...
			return 'TapeRecorder/sounds/' + this.action + '.' + type;
