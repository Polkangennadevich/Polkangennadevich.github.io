$(function(){
	...
	lightswitch.click(function(){
		if(lightbulb.hasClass("off")){
			...
			logoshadow.stop().fadeTo(670,setOpacity(shadowDistance));
			lightbulb2.stop().fadeTo(670,1);
			statustext.text(offtxt);
		}else{
			lightbulb.addClass("off");
			lightswitch.css("backgroundPosition","-80px 0");
			logoshadow.stop().fadeTo(670,0);
			lightbulb2.stop().fadeTo(670,0);
	...
	$("table.sh").hoverIntent(function(){
		$(this).stop().animate({width:"670px"},200,function(){ $(this).css("overflow","visible"); });
	},function(){
        $(this).stop().animate({width:"670px"},200,function(){ $(this).css("overflow","auto"); });
	});	
});
/**
* hoverIntent
